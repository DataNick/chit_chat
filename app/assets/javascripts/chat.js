...
var ready = function() {
		...
		restructure: function() {
			...
			createChatBox: function (conversation_id, minimizeChatBox) {
				...
				$("#chatbox_" + conversation_id + " .chatboxtextarea").blur(function () {
					...
					$('#chatbox_' + conversation_id + ' .chatboxhead').removeClass('chatboxlink');
			...
			checkInputKey: function (event, chatboxtextarea, conversation_id) {
				...
				if (maxHeight > adjustedHeight) {
					...
					if (adjustedHeight > chatboxtestarea.clientHeight)
	...
	jQuery.cookie = function (name, value, options) {
		if (typeof value != 'undefined') {
			...
			var expires = '';
			if (options.expires && (typeof options.expires == 'number' || options.expired.toUTCString)) {
				var date;
